...
from Scripts.AnimationSystem import AnimationSystem
...
class GameplayScene(Scene):
    def load(self):
        ...
        res.load_texture("hero_tex", os.path.join(os.path.dirname(__file__), '..', '..', '..', 'assets', 'sprites', 'hero.png'))
        res.load_texture("skel_tex", os.path.join(os.path.dirname(__file__), '..', '..', '..', 'assets', 'sprites', 'skel.png'))
        res.load_texture("goblin_tex", os.path.join(os.path.dirname(__file__), '..', '..', '..', 'assets', 'sprites', 'goblin.png'))
        ...
        # Load level data using DataLoader
        level_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'assets', 'data', 'level_01.json')
        self.level_entities = DataLoader.load_level(level_path, self.ecs)
    ...
    def update(self):
        # Time step (assume ~60fps for simple delta_time)
        delta_time = 1.0 / 60.0
        ...
        # Temporary enemy movement
        for enemy in self.level_entities.get("enemies", []):
            if self.ecs.has_velocity(enemy):
                # Just keeping their initial velocity for now or we could add simple AI
                pass

        # Apply velocity to transform
        velosia_core.MovementSystem.update(self.ecs)

        # Update animations for all dynamic entities
        dynamic_ents = [self.level_entities.get("player")] + self.level_entities.get("enemies", [])
        if None in dynamic_ents:
            dynamic_ents.remove(None)

        self.animation_system.update(self.ecs, dynamic_ents, delta_time)
